define(['jquery',"Sortable","functions","commonutilities"], function($, Sortable,myFunctions) {
...
init=function(id,FX,opt)
	...
	addCSS("toolbar/toolbar.css");
	
	
	_onChange=opt.onchange;
	for(var i in FX.columns)
		{
			if(FX.columns[i].id=="value"){HIDDEN.push({value:FX.columns[i].id,label:FX.columns[i].title[lang]})}
		else if (FX.columns[i].subject!="time" ){ROWS.push({value:FX.columns[i].id,label:FX.columns[i].title[lang]});}
...
{
var parent=this;
$("#"+id_container).append("<fieldset class=\"myFieldset\"><ul id=\""+id_container+"_AGG\"><div class=\"title\">AGG</div></ul></fieldset><fieldset class=\"myFieldset\">"+
"<ul id=\""+id_container+"_ROWS\"><div class=\"title\">ROWS</div></ul></fieldset><fieldset class=\"myFieldset\">"+
"<ul id=\""+id_container+"_COLS\"><div class=\"title\">COLS</div></ul></fieldset>");
